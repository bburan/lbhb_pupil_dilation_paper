...
parameters {
    ...
    real threshold_alpha;
    real threshold_beta;
    vector<lower=0, upper=1>[n_cells] threshold_cell_t;
...
transformed parameters {
    ...
    threshold_cell = threshold_cell_t * 100 + threshold_delta_cell / 2;
    threshold_mean = exp(threshold_alpha)/(exp(threshold_alpha) + exp(threshold_beta));
    
    slope_cell_pupil = slope_cell + slope_delta_cell;
    threshold_cell_pupil = threshold_cell - threshold_delta_cell/2;
...
model {
    ...
    slope_delta_mean ~ normal(0, 0.01);
    slope_delta_sd ~ normal(0, 0.01);
    slope_delta_cell ~ normal(slope_delta_mean, slope_delta_sd);
    
    threshold_alpha ~ normal(0.7, 0.1);
    threshold_beta ~ normal(0.7, 0.1);
    threshold_cell_t ~ beta(exp(threshold_alpha), exp(threshold_beta));
